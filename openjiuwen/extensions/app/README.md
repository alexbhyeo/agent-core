...
- `map_tools.py` -- `geocode_place`/`show_map`, for resolving real places (Google Places API, incl. rating/photo when available) and rendering them as an interactive map (Google Maps JavaScript API, via `/map-embed`) with tappable pins whose info window shows the place's name, photo, and rating.
...
- `server.py` -- FastAPI app factory + `/ws` endpoint + entrypoint. Also serves two small HTML pages the client's custom WebView components load directly: `/youtube-embed` (wraps a YouTube URL in a real `<iframe>`) and `/map-embed` (an interactive Google Maps JavaScript API page with default markers, see `map_tools.py`).
